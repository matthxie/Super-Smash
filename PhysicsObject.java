...
public class PhysicsObject extends JPanel {	
	...
	public void draw(Graphics g) {	//The object's own draw method (this is what canvas from the physics class calls to draw onto panel)
		...
		if(!deadRightNow) {
			if(this.lastX > Physics.width || this.lastX < 0 || this.lastY > Physics.height) {
				...
				deadRightNow=true;
				System.out.println("DEADEADAEDEADAEADEADEADEAEDAEDAEDAEDAEDAEDADEADEADEAEDA");
			}
	...
	//			if(Physics.physicsObjectList.get(i) != this) {
	//				PhysicsObject temp = Physics.physicsObjectList.get(i);
	//				//				if(this.lastX+this.objectW>= temp.lastX && this.lastX <= temp.lastX) {
	//				//					if((this.lastY+this.objectH<=temp.lastY+temp.objectH && this.lastY>= temp.lastY)) {
	//				//						return true;
	//				//					}
	//				//
	//				//				}
	...
	public boolean platformCollision() {
		for(int i = 0; i < Physics.platformList.size(); i++) {
			...
			if(temp.x <= lastX + objectW && temp.x+temp.width >= lastX) {
				if(temp.y <= lastY+objectH && (temp.y+temp.height) >= lastY+objectH) {
					return true;
				}

			}
			if(this.lastX >= temp.x && this.lastX<=temp.x+temp.width) {
				if(this.lastY <= temp.y) {//+ this.objectH 
					if(this.lastY + this.objectH+(fallSpeed * fallingTime)>= temp.y) {
						this.lastY = temp.y-this.objectH;
						return true;
					}
				...
				//			Platform temp =Physics.platformList.get(i);
				//			if((this.lastX > temp.x && this.lastX<temp.x+temp.width)) {
				//				if(this.lastY +this.objectH <= temp.y) {
				//					if(this.lastY + this.objectH+this.fallSpeed>= temp.y) {
				//						System.out.println("collideedddd");
				//
				//						return true;
				//					}
				//
	...
	//	Platform temp =Physics.platformList.get(i);
	//	if((this.lastX > temp.x && this.lastX<temp.x+temp.height)) {
	//		if(this.lastY +this.objectH <= temp.y) {
	//			if(this.lastY + this.objectH+this.fallSpeed>= temp.y) {
	//				return true;
	//			}
